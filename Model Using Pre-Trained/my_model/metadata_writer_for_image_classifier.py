...
_MODEL_INFO = {
    "model_unquant.tflite":
        ModelSpecificInfo(
            name="A TFLite model for detecting BISINDO Alphabets.",
            version="v1",
            image_width=224,
            image_height=224,
            image_min=0,
...
class MetadataPopulatorForImageClassifier(object):
    ...
    def _create_metadata(self):
        ...
        label_file.description = "Labels for BISINDO Alphabets that the model can recognize."
...
def main(_):
    ...
    if model_basename not in _MODEL_INFO:
        raise ValueError(
            "The model info for WavEd is not defined yet.".format(model_basename))
